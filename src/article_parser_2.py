...
DATES = {'Monday','Tuesday','Wednesday','Thursday','Friday','Saturday','Sunday','January','February','March','April','May','June','July','August','September','October','November','December','Jan','Feb','Mar','Apr','Jun','Jul','Aug','Sep','Oct','Nov','Dec'}
...
def add_alias(alias,article_aliases,entity,start,article_aliases_list,file_entity):
    ...
    if (alias in mostly_upper_articles and entity != file_entity) and alias != entity and (alias not in article_aliases or alias not in article_aliases[alias]):
        return
...
def find_entity_for_mention(start,mention,article_aliases,entities,all_found_aliases,file_entity,unknown_persons,all_entities):
...
def annotate_entities(sentence,article_aliases,article_aliases_list,sentence_starters,start_of_sentence,entities,all_found_aliases,file_entity,all_entities,unknown_persons):
    found_entities = []
    end_of_sentence = ''
    if len(sentence) == 0:
        return sentence
    ...
    for m in re.finditer(RE_ACRONYM, sentence):
        ...
        if len(uppercase_letters) >= len(acronym) and uppercase_letters_string[-len(acronym):] == acronym:
            ...
            entity, annotation_type = find_entity_for_mention(uppercase_letters[-len(acronym)],actual_entity, article_aliases, entities, all_found_aliases,
                                                              file_entity, unknown_persons, all_entities)

            if len(entity) == 0:
                entity = actual_entity
            found_entities.append((uppercase_letters[-len(acronym)], len(actual_entity), entity, annotation_type,actual_entity))
            sentence = sentence[:uppercase_letters[-len(acronym)]] + placeholder + sentence[uppercase_letters[-len(acronym)] + len(actual_entity):]
        ...
        while True:
            ...
            if pos_alias > -1:
                ...
                if before_string is None or after_string is None:
                    ...
                    sentence = sentence[:pos_alias + len(before_string)] + placeholder + sentence[pos_alias + len(string_to_match) - len(after_string):]
            else:
                break
    ...
    for j in range(len(tokens)):
        ...
        if len(token) > 0 and token[0].isupper() and not is_number(token):
            is_start_of_new_sentence = (j == first_word_index and start_of_sentence) or sentence[len(previous)-2:len(previous)] == ': '
            ...
            if token in DATES:
                previous += ' ' + token
                continue
            if token not in unknown_persons and is_start_of_new_sentence and not next_word_upper and (token.lower() in sentence_starters or token not in article_aliases) and token not in mostly_upper_articles:
                previous += ' ' + token
                continue

            if len(token) == 1 and len(sentence) > len(previous) + len(token) and sentence[len(previous) + len(token)] == '.':
    ...
    while j < len(found_entities)-1:
        ...
        if len(between) == 0 or between == ' ' or between == ' & ' or between == '-' or re.sub(r'\W+', '', between).strip() in {'de','of','von','van'} or mention_in_article_aliases or mention_in_aliases or mention_in_redirects or mention_in_aliases_reversed:
    ...
    for j in range(len(found_entities)):
        ...
        if len(found_entities[j]) == 5:
            ...
            entity, annotation_type = find_entity_for_mention(start,mention,article_aliases,entities,all_found_aliases,file_entity,unknown_persons,all_entities)
        ...
        if pos > -1:
            ...
            if start > -1 and end > -1:
                ...
                if all.count('ANNOTATION') == 1:
                    ...
                    content = content[:start] + '[[' + entity + '|' + alias + '|' + annotation + ']]' + content[end+2:]
...
def break_content(content,file_entity):
    ...
    for line in lines:
        ...
        while i < len(sent_tokenize_list):
            if i > 0 and len(sent_tokenize_list[i]) > 0 and sent_tokenize_list[i].lstrip()[0].isalpha() and sent_tokenize_list[i].lstrip()[0].islower():
        ...
        while i < len(sent_tokenize_list):
            ...
            if len(sent_tokenize_list[i]) > 0 and i != previously_added:
                previously_added = i
                if sent_tokenize_list[i] in file_entity and len(file_entity) > len(sent_tokenize_list[i]) and i < len(sent_tokenize_list) - 1:
...
def process_file(filename,output_filename):
    with open(filename) as f:
        if filename in filename2title:
            ...
            file_entity = filename.split('/')[-1][:-4].replace('_', ' ')
            ...
            for content in lines:
                ...
                while True:
                    ...
                    if start > -1 and end > -1 and end > start:
                        ...
                        previous_part = annotate_entities(previous_part, article_aliases, article_aliases_list, sentence_starters,start_of_sentence,entities,all_found_aliases,file_entity,all_entities,unknown_persons)
                        ...
                        if 'TEMPLATE' in mention:
                            ...
                            if entity in mostly_upper_articles and is_upper(alias) :
                                ...
                                if entity not in aliases_reversed:
                                    first_part = content[:idx] + previous_part + '[[' + entity + '|' + alias + '|NOENTITY_ANNOTATION]]'
                                elif alias not in aliases_reversed[entity]['dict'] or aliases_reversed[entity]['dict'][alias] <= 1:
                                    first_part = content[:idx] + previous_part + '[[' + entity + '|' + alias + '|RARE_ANNOTATION]]'
                                else:
                                    first_part = content[:idx] + previous_part + '[[' + entity + '|' + alias + '|ANNOTATION]]'
                ...
                sentence = annotate_entities(sentence, article_aliases, article_aliases_list, sentence_starters, start_of_sentence,entities,all_found_aliases,file_entity,all_entities,unknown_persons)
...
def process(article,file_directory):
    ...
    process_file(article, output_filename)
